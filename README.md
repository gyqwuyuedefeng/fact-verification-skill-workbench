...
现场只选择以下一条路径，不把不同路径的结果混作同一次运行证据：
...
从零实跑并完成三版本评测、影子、晋升与回滚后，已经通过页面“导出演示快照（ZIP）”保存本机恢复点：

```text
/mnt/f/IdeaProjects/hsmap/.playwright-mcp/workbench-state-snapshot.zip
Windows 文件选择器：F:\IdeaProjects\hsmap\.playwright-mcp\workbench-state-snapshot.zip
SHA-256: 74b091c7e660d44923f962add9f84384c48053137aec0c6e80797c7a824bcd01
```

需要恢复时，先为当前状态另行导出备份，再在“演示数据管理”清空状态，选择上述 ZIP 并点击“导入自定义快照”。恢复后的七表计数依次为：主张 27、核验运行 12、核验任务 11、证据快照 92、发布绑定 6、Skill 版本 2、评测运行 4。该快照包含真实运行证据和上传材料，只允许本机演示恢复，不进入 Git、公开提交包或无关渠道。
