...
async function deleteDraft() {
  ...
  const confirmed = globalThis.confirm(`确认删除 DRAFT「${store.currentDraft.changeSummary}」？删除后无法恢复。`)
  if (!confirmed) return
...
async function generateChangeSummary() {
  ...
  await store.compareVersions(targetVersionId.value, baseVersionId.value)
}
</script>
...
<template>
  <section>
    ...
    <div class="page-heading">
      <div>
        <h1>Skill 版本实验室</h1>
        <p class="page-lead">点击任意版本查看完整内容：DRAFT 可编辑可删除，冻结版本永久只读。</p>
      </div>
      <button
        ...
        @click="createDraft"
      >
        {{ store.selectedVersionId ? '从已选版本克隆 DRAFT' : store.versions.length ? '请先选择一个版本' : '新建初始 DRAFT' }}
      </button>
    </div>
    ...
    <div class="skill-layout">
      <section class="panel skill-editor">
        <div class="panel-heading">
          ...
          <div>
            <strong>{{ store.currentDraft ? 'DRAFT 编辑' : store.selectedContent ? '版本内容（只读）' : '选择版本查看内容' }}</strong>
            <small>SKILL.md + references + 变更说明</small>
          </div>
        </div>
        ...
        <label class="field-label">SKILL.md</label>
        <textarea
          v-model="skillMarkdown"
          class="code-editor"
          rows="13"
          placeholder="创建 DRAFT 后，在这里粘贴或调整 SKILL.md"
          :readonly="!store.currentDraft"
        ></textarea>
        <label class="field-label">references JSON</label>
      ...
      <section class="panel version-list-panel">
        <div class="panel-heading">
          ...
          <div><strong>版本历史</strong><small>DRAFT / CANDIDATE / STABLE / ARCHIVED</small></div>
        </div>
        ...
        >
          ...
          <code>{{ version.contentHash?.slice(0, 12) ?? 'editable' }}</code>
        </button>
      </section>
    </div>
    ...
    <section class="results-section version-card-panel">
      <div class="results-heading">
        ...
        <span class="gate-chip" :class="store.card?.gateStatus.toLowerCase()">GATE {{ store.card?.gateStatus ?? 'PENDING' }}</span>
      </div>
      <div v-if="!store.card" class="empty-results">选择一个冻结版本查看版本卡。</div>
      <div v-else class="version-card-grid">
        <div>
          <small>VERSION</small><strong>{{ store.card.version }}</strong>
        </div>
        <div>
          <small>PARENT</small><strong>{{ store.card.parentVersion ?? 'ROOT' }}</strong>
        </div>
        <div>
          <small>CONTENT HASH</small><code>{{ store.card.contentHash }}</code>
        </div>
        <div>
          <small>CHANGE</small><strong>{{ store.card.changeSummary }}</strong>
        </div>
        <div class="card-wide">
          <small>REGISTER</small>
          <strong v-if="store.card.gateStatus === 'PASS'">离线门禁通过，可以注册 Candidate</strong>
      ...
      <div class="comparison-controls">
        <label>
          ...
          <select v-model="targetVersionId" class="text-input" data-test="target-version">
            ...
            <option v-for="version in store.versions.filter((item) => item.status !== 'DRAFT')" :key="version.id" :value="version.id">{{ version.version }} · {{ version.status }}</option>
          </select>
        </label>
        <label>
          <span>基准版本</span>
